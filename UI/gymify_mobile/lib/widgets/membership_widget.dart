...
class _MembershipWidgetState extends State<MembershipWidget> {
  ...
  Future<void> _submit() async {
    ...
    if (_isActive &&
      ...
      await ConfirmDialogs.paymentErrorDialog(
        context,
        message: "Ne možete promijeniti plan dok je trenutna članarina aktivna.",
      );
      return;
    }
    ...
    try {
      ...
      if (stripeSuccess == null) {
        ...
        await ConfirmDialogs.paymentFailedDialog(
          context,
          message: "Plaćanje nije uspješno pokrenuto.",
        );
        return;
      }
      ...
      final refreshed = await _paymentProvider.getById(stripeSuccess.paymentId);
  ...
  bool _ctaEnabled() {
  if (_selectedPlan == null || _processing) return false;
        ...
        child: ListView(
          ...
          children: [
            ...
            ..._plans.map((p) {
              ...
              final selectable = _canSelectPlan(p);
                  ...
                  onTap: () {
                    if (!selectable) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            "Ne možete promijeniti plan dok je članarina aktivna.",
                          ),
                        ),
                      );
                      return;
            ...
            if (_hasMembership && _isActive)
              const _HintBox(
                text:
                    "Dok je članarina aktivna, nije moguće mijenjati plan. Možete samo produžiti trenutni paket.",
              ),
          ],
        ),
      ),
...
class _StatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!hasMembership) {
      ...
      subtitle = "Možeš produžiti trenutni paket.";
    } else {
